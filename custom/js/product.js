...
$(document).ready(function() {
	...
	$("#addNewBrandBtn").click(function(){
		...
		if(newBrandName == "") {
			...
			$("#newBrandName").after('<p class="text-danger">Product Name field is required</p>');
			$('#newBrandName').closest('.form-group').addClass('has-error');
		}else {
			// remov error text field
			$("#newBrandName").find('.text-danger').remove();
			// success out for form
			$("#newBrandName").closest('.form-group').addClass('has-success');
		}	// /else
		if (newBrandName) {
			$.ajax({
				url: 'php_action/addNewBrand.php',
				type: 'post',
				data: {newBrandName: newBrandName},
				dataType: 'json',
				success:function(response) {
					if(response.success == true) {

						$("#brandName").load("php_action/load-brand.php");
						$("#newBrandName").val("");
						$("#errorNewBrand").val("Added");
						$(".text-danger").remove();
						// remove from-group error
						$(".form-group").removeClass('has-error').removeClass('has-success');

					}else if(response.success == false) {
						$("#errorNewBrand").val("Already Added");
					}
				}
	...
	$("#addNewCategoryBtn").click(function(){
		...
		if(newCategoryName == "") {
			$(".text-danger").remove();
			$(".form-group").removeClass('has-error').removeClass('has-success');
			$("#newCategoryName").after('<p class="text-danger">Product Name field is required</p>');
			$('#newCategoryName').closest('.form-group').addClass('has-error');
		}else {
			// remov error text field
			$("#newCategoryName").find('.text-danger').remove();
			// success out for form
			$("#newCategoryName").closest('.form-group').addClass('has-success');
		}	// /else
		if (newCategoryName) {
			$.ajax({
				url: 'php_action/addNewCategory.php',
				type: 'post',
				...
				success:function(response) {
					if(response.success == true) {
						...
						$("#categoryName").load("php_action/load-category.php");
						$("#newCategoryName").val("");
						$("#errorNewCategory").val("Added");
						$(".text-danger").remove();
						// remove from-group error
						$(".form-group").removeClass('has-error').removeClass('has-success');

					}else if(response.success == false) {
						$("#errorNewCategory").val("Already Added");
					}
				}
