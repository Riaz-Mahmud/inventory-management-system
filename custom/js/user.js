...
$(document).ready(function() {
	...
	$("#addUserModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitUserForm").unbind('submit').bind('submit', function() {
			...
			if(userType == "") {
				$("#uType").after('<p class="text-danger">User Type field is required</p>');
				$('#uType').closest('.form-group').addClass('has-error');
			}	else {
				// remov error text field
				$("#uType").find('.text-danger').remove();
				// success out for form
				$("#uType").closest('.form-group').addClass('has-success');
			}	// /else


				// /else

			if(upassword && userName && userType) {
		...
		$.ajax({
			...
			// alert(response.product_image);
				...
				$("#editUserForm").unbind('submit').bind('submit', function() {
					...
					if(userpassword && username && userType) {
						// submit loading button
